...
router.get('/seenStatus', auth, async (req, res) => {
  try {
    ...
    const shapedData = req.user.seenStatus.map((seenStatus) => {
      return { [seenStatus.film]: seenStatus.seenStatus };
    });
...
router.get('/seenStatus/:id', auth, async (req, res) => {
  try {
    const seenStatus = await SeenStatus.findOne({
      _id: req.params.id,
      owner: req.user._id,
    });
    if (!seenStatus) {
...
router.post('/seenStatus', auth, async (req, res) => {
  ...
  try {
    ...
    toUpdateKeys.forEach(
      (itemToUpdate) => (seenStatus[itemToUpdate] = req.body[itemToUpdate])
    );
    await seenStatus.save();
    res.send(seenStatus);
  } catch (e) {
...
router.delete('/seenStatus/:id', auth, async (req, res) => {
  try {
    const seenStatus = await SeenStatus.findOneAndDelete({
      _id: req.params.id,
      owner: req.user._id,
    });
